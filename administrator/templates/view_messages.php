...
<form action="view.php?mode=messages" method="post" name="messages_time_sort">
<select name="messages_sort" onchange="javascript:document.messages_time_sort.submit();">
<?php
foreach($order as $k=>$v){
	if($_SESSION["messages_sort"] == "$k"){
?>
...
<tr>
<td colspan="3" style="margin:0;padding:0 0 10px 0;line-height:30px;" align="right">
<div class="f_detail" style="width:780px;margin:0 auto;border-bottom:1px dashed #666666;"><a href="delete.php?mode=messages&id=<?php echo $row_reply["id"];?>" onclick="return del();" target="_self">[删除该回复]</a></div>
</td>
<?php
}
?>
...
<div class="f_detail" style="float:right;"><a href="delete.php?mode=messages&id=<?php echo $v["id"];?>" onclick="return del();" target="_self">[删除该留言]</a></div>
<?php
if($row_reply == "nothing"){
?>
<div class="f_detail" style="float:right;"><a href="javascript:void(0)" id="reply_form<?php echo $i;?>" onClick="show_reply_form(<?php echo $i;?>,'<?php echo $v["id"];?>')" target="_self" style="color:#00F;">[回复该留言]</a></div>
<?php	
}
?>
</td>
