...
<div id="show_locked<?php echo $i;?>" style="float:left;">
<?php
if($v["locked"]==0){
?>
<div class="f_detail2"><a href="javascript:void(0)" onclick="locked_ajax(<?php echo $i;?>,<?php echo $v["id"];?>, 1)">[锁定视频]</a></div>
<?php
} else {
?>
<div class="f_detail"><a href="javascript:void(0)" onclick="locked_ajax(<?php echo $i;?>,<?php echo $v["id"];?>, 0)">[解锁视频]</a></div>
<?php
}
?>
</div>
...
<div style="font-size:14px; color:#666; height:30px;">输入密码：<input type="password" id="locked_password" name="locked_password" /></div>
<div class="reply_form_button"><button type="button" onclick="update_locked_password();">更新</button>&nbsp;<button type="button" onclick="close_form();">关闭</button></div>
</form>
</div>
</div>
</body>
...
var _order
var _locked
function locked_ajax(order,v_id,locked){
	if(locked == 0){
		_order = order
		_v_id = v_id
		_locked = locked
...
function update_locked_password(){
	if($("#locked_password").val()==""){
		alert("请输入密码！")
		return false;
	}	
	$.ajax({
		   type: "POST",
		   url: "ajax/update_vedio_locked.php?id="+_v_id+"&order="+_order+"&locked="+_locked,
