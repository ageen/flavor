...
<?php
	...
	if($row_vedio["locked"]==1){
		if(isset($_SESSION["unlock"])&&in_array($row_vedio["id"], $_SESSION["unlock"])){
			true;
		} else{
?>
<div style="width:300px;margin:0 auto 0 auto; padding:30px;background:#FFF;">
<form onSubmit="return false;" method="post" id="pass_form">
<div style="font-size:14px;font-weight:bold;color:#F00;line-height:50px;">该视频锁定</div>
<div style="height:50px;"><input type="password" name="locked_password" id="locked_password" /></div>
<div style="height:50px;"><button type="button" onClick="ajax_vedio_pass(<?php echo $row_vedio["id"];?>)" style="font-weight:bold;">解锁</button></div>
		...
		$.ajax({
			...
			success: function(msg){
				if(msg == "error"){
					...
					window.location.href="detail.php?mode=vedio&id="+id
				}
			}
		});
	}
...
?>
            'autoplay':'false',
			//'clickTAG':'http://localhost/flavor/detail.php?mode=vedio&id=<?php echo $row_vedio["id"];?>',
